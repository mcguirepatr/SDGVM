...
!       gfortran states_convertSDGVM_v6a.f90 -o states_convertSDGVM_v6a `nf-config --fflags --flibs`

!versions:
!    v5b: working version that replicates original R code
!    v6a: enhancement to keep track of both primary & secondary tree FTs

      PROGRAM states_convertSDGVM_v6a
      USE netcdf
      IMPLICIT NONE
      ...
      INTEGER, PARAMETER :: NX = 720, NY = 360, NT = 1172, NV = 14, NV2 = 8
      REAL, PARAMETER    :: misval = 1e19
      INTEGER, PARAMETER :: NE = 10, NE2 = 17, NS = 15 
     ...
     !     'urban','secmb', 'secma'/)
      CHARACTER(LEN=7),PARAMETER :: varname_sdgvm(NS)=(/'  BARE',' Ev_Bp',' Dc_Bp',' Ev_Np',' Dc_Np', &
          ' Shrup','    C3','    C4','C3crop','C4crop',' Ev_Bs',' Dc_Bs',' Ev_Ns',' Dc_Ns',' Shrus'/)
    ...
    FUNCTION JOIN_HYDE(v) RESULT( result_join_hyde ) 
      ...
      ! and combines into a 15 element vector of SDGVM PFTs
      ...
      ! SDGVM PFTs: 1 BARE, 2 Ev_Bp, 3 Dc_Bp, 4 Ev_Np, 5 Dc_Np, 6 Shrup, 7 C3, 8 C4, 9 C3crop, 10 C4crop,
      !                    11 Ev_Bs,12 Dc_Bs,13 Ev_Ns,14 Dc_Ns,15 Shrus
      ...
      ELSE IF((SUM(v) > 0.0) .AND. (SUM(v) < 2000.0) ) THEN 
       ...
       ELSE 
        result_join_hyde = (/ NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA /) 
       END IF 
    ...
    END PROGRAM states_convertSDGVM_v6a
