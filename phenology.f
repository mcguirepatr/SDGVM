...
*----------------------------------------------------------------------*
      DO ij=1,rootls-1
        rtemp  = respref*rootv(rootls-ij)
        resp   = resp + rtemp
        rootresp = rootresp + rtemp
        s_rr     = s_rr + rtemp
        rootnpp  = rootnpp - rtemp
        rootv(rootls-ij) = rootv(rootls-ij) - rtemp
      ENDDO
...
*      xdaynpp = xdaynpp - stemnppr*respref
      ...
      s_sr = resp
      DO ij=1,stemls-1
        rtemp  = respref*stemv(stemls-ij)
        resp   = resp + rtemp
        stemresp = stemresp + rtemp
        stemnpp  = stemnpp  - rtemp
        stemv(stemls-ij) = stemv(stemls-ij) - rtemp
      ENDDO
      s_sr = resp - s_sr
...
*  - I can't see where 5 years are involved in the calculations APW                                                                     *
*----------------------------------------------------------------------*
...
* This is essentially the LAI control.                                 *
*----------------------------------------------------------------------*
            !this occurs only on the day of budburst
            ...
            !maint is the annual turnover rate of leaves
            maint = max(1.0d0,(real(leafls)/360.0d0))
     ...
     &tsuma = -leafmol*1.25d0/maint*p_opt
            ...
            !tsumam = 0.0d0*tsumam + 1.0d0*tsuma ! commented out APW these tow lines are unnecessary replaced below
            !stemfr = stemfr + tsumam*p_laimem
...
*----------------------------------------------------------------------*
            ...
            ELSE
              nppstor2 = nppstore*0.25d0
              !reduces the value of lai which is passed to next year by 5%  
              stemfr = stemfr*0.95
            ENDIF
...
*----------------------------------------------------------------------*
* Set LAI increase - from day after budburst onwards.                                                    *
*----------------------------------------------------------------------*
      IF ((bb.GT.0).AND.(bbgs.LT.gs).AND.(nppstore.GT.1.0d0)) THEN
        !daily LAI increment
        !nppstorx - nppstor2 is the amount of leaf allocated C remaining
        !i.e. once nppstorx = nppstor2 leaf allocation for the year ceases
