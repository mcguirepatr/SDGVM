     ...
     &seed1,seed2,seed3,swrv,read_par)
      IMPLICIT NONE
      REAL*8 tmpv(500,12,31),humv(500,12,31),prcv(500,12,31)
      REAL*8 swrv(500,12,31),mswrv(500,12)
      REAL*8 mtmpv(500,12),mhumv(500,12),mprcv(500,12),lat,lon
      INTEGER yr0,yrf,seed1,seed2,seed3,nbrain,blank,du,i,read_par
...
c local variables
      ...
      DO year=1,yrf-yr0+1
        DO mnth=1,12
          ...
          mswrv(year,mnth) = swrv(year,mnth,1)
        ENDDO
      ENDDO
...
c generate the humidity
     ...
     &correction
        ...
        IF(read_par.eq.1) THEN
          CALL MEAN_PRES(mswrv(year,:),swrv(year,:,:))
        ENDIF
...
*----------------------------------------------------------------------*
      ...
      REAL*8  mnthly(12), daily(360), daily_new(360), corr(12), sumdiff
      REAL*8  swrv(12,31)
     ...
     &daily((mnth-1)*30+day))
          ...
          DO mnth=1,12
            DO day=1,30
              daily((mnth-1)*30+day) = daily((mnth-1)*30+day)
     & + corr(mnth)                    
            ENDDO
          ENDDO
        ENDDO
...
c local variables
      ...
      REAL*8 tmp_yesterday, hum_yesterday,xx(4,4)
      REAL*8 tmp_autocorr_month,hum_autocorr_month
      REAL*8 corr(12),dsw(360),dsw_new(360),sumdiff
...
c      print*, stinput(1:blank(stinput))//'/maskmap.dat'
c	MMN=0
c	read(fno+1,'(96i6)',iostat=MMER) MMI	
c	print*, MMI(16)
c	do 
c	read(fno+1,'(96i6)',iostat=MMER) MMI
c	MMN=MMN+1
c		if (MMER<0) then
c		print*, 'end of file etc'
c		stop
c		endif
		
c		if (MMER>0) then
c		print*, 'formatting error'
c		stop
c		endif
...
*----------------------------------------------------------------------*
* Find the four sites closest to the desired.in the maskmap file       *
*----------------------------------------------------------------------*
      DO ii=1,4
        DO jj=1,4
          row = int(rrow)+jj-1
...
*----------------------------------------------------------------------*
     ...
     &form='formatted',status='old',iostat=kode)
        ...
        IF(read_par.eq.1) THEN
        OPEN(8,file=fname8,access='direct',recl=recl3,        !added by APW 
     &form='formatted',status='old',iostat=kode)
        IF (kode.NE.0) THEN
...
c swr interpolated smoothly to preserve the monthly mean added by APW
c mean preserving interpolation Rymes & Myers 2001

        IF(read_par.eq.1) THEN
c	print*, mswrv(year-yr0+1,:)
c	print*, swrv(year-yr0+1,:,:)
          CALL MEAN_PRES(mswrv(year-yr0+1,:),swrv(year-yr0+1,:,:))

c             IF(read_par.eq.1) THEN 
c             corr(:) = 0.0d0
c             iter = 360
c             DO mnth=1,12
c               DO day=1,30  
c               dsw((mnth-1)*30+day) = mswrv(year-yr0+1,mnth)
c               ENDDO
c             ENDDO
...
c               DO day=1,iterc
c                 IF(day.eq.1) then
c                   dsw_new(day) = (dsw(iter)+dsw(day)+dsw(day+1))/3
c                 elseif(day.eq.iter) then
c                   dsw_new(day) = (dsw(day-1)+dsw(day)+dsw(1))/3
c                 else
c                   dsw_new(day) = (dsw(day-1)+dsw(day)+dsw(day+1))/3
c                 endif                    
...
c                  sumdiff = 0.0d0
c                  DO day=1,30
c                    sumdiff = sumdiff+(mswrv(year-yr0+1,mnth)-
c     &dsw((mnth-1)*30+day))
c                  ENDDO
c                  corr(mnth) = sumdiff/iter
c                ENDDO

c                DO mnth=1,12
c                  DO day=1,30
c                    dsw((mnth-1)*30+day) = dsw((mnth-1)*30+day)
c     & + corr(mnth)                    
c                  ENDDO
c                ENDDO
c              ENDDO
...
c check interpolation
     ...
     &(swrv(year-yr0+1,mnth,day))
       ...
       !if(read_par.eq.1) then
       !open(fno+9,file='test outputs/sw_interp_check.dat',
      !&access='append')
       !write(fno+9,'(a5,1x,12f7.2)') 'interp',
      !&(mswrvcheck(month),month=1,12)
       !write(fno+9,'(a5,1x,12f7.2)') 'read',
      !&(mswrv(1,month),month=1,12)
       !close(fno+9) 
       !endif 
