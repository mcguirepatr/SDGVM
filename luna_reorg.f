     ...
     &surfalb_inst, solarabs_inst, waterstate_inst, frictionvel_inst)
        ...
        !fnps         =  0.15_r8 - fnps appears to do nothing
        !call t_startf('LUNA')
        !do f  =  1,fn
        !p  =  filterp(f)
        !ft =  patch%itype(p)     
        ...
        !        if(tlai > 0.0) then
               ...
               ! par240_z is 10-day mean, layer specific absorbed PAR in wm-2   
               ...
               ! PARTop is 10-day mean, incident PAR in top canopy layer per unit leaf in umol m-2s-1
        PARTop =  par240d_z(1)/rabsorb
        
        !*****************************************************************************88                      
        ! CUT this section into the LUNA subroutine
        !*****************************************************************************88                      
               ...
               ! z is canopy layer
               ! only optimize if there is growth and it is C3 plants
        !                if (nint(c3psn(ft)) == 1)then
        !                     if(gpp_day(p)>0.0 )then   
        !                      !-------------------------------------------------------------
        !                      ! z is canopy layer
        !                      do z = 1, nrad
        !                         if(tlai_z(z)>0.0_r8)then
        !                            !qabs   = par240d_z(z)/rabsorb
        !                            !PARi10 =  qabs * 4.6_r8                 
        !                            !PARi10 is 10-day mean, incident PAR in canopy layer per unit leaf in umol m-2s-1
        ...
        !                         !endif
        !                         ! SLA is fixed through the canopy in SDGVM
        !                         FNCa = relCLNCa*SNCa                !(Eq A4)
        !                      !enddo
        !                      
        !                      !----------------------------------------------------------------------
        ...
        !                         PNlc_z(z)   = PNlcopt
        !
        !                         if(enzs_z(z)<1.0) then
        !                            enzs_z(z) = enzs_z(z)* (1.0_r8 + max_daily_pchg)
        !                         endif
        !                         !nitrogen allocastion model-end  
        !                         !-----------------------------------------------------------------------------------------------------   
        !                         if(isnan(vcmx25_z(z)).or. vcmx25_z(z)>1000._r8 .or. vcmx25_z(z)<0._r8)then
        !                             write(iulog, *) 'Error: Vc,mx25 become unrealistic (NaN,>1000, or negative) for z=', z
        !                             write(iulog, *) 'LUNA env:',FNCa,forc_pbot10, relh10, CO2a10, O2a10, PARi10, PARimx10, rb10v, &
        !                                  hourpd, tair10, tleafd10, tleafn10
        !                             call endrun(msg=errmsg(__FILE__, __LINE__))
        !                         endif
        !                         if(isnan(jmx25_z(u)).or.jmx25_z(z)>1000._r8 .or. jmx25_z(z)<0._r8)then
        !                             write(iulog, *) 'Error: Jmx25 become unrealistic (NaN,>1000, or negative)for z=', z
        !                             write(iulog, *) 'LUNA env:', FNCa,forc_pbot10, relh10, CO2a10, O2a10, PARi10, PARimx10, rb10v, &
        !                                  hourpd, tair10, tleafd10, tleafn10
        !                             call endrun(msg=errmsg(__FILE__, __LINE__))
        !                
        ...
        !*****************************************************************************88                      
        ! end CUT section into the LUNA subroutine
        !*****************************************************************************88                      
        
        
                   ! decay during drought or winter i.e. GPP = 0.0
        !                    else 
        
        !*****************************************************************************88                      
        ! CUT section into the LUNA_nogrowth subroutine
        !*****************************************************************************88                      
        ...
        !                      end do              
        !*****************************************************************************88                      
        ! end CUT section into the LUNA_nogrowth subroutine
        !*****************************************************************************88                      
     ...
     &ftg1,z)
         ...
         !call ASSIMILATION_CALC(forc_pbot10, tleafd10, relh10, CO2a10, O2a10, rb10, Vcmax, JmeanL, ci, Kc, Kj, A)
        else
         Wc = Kc * Vcmax
         Wj = Kj * JmeanL
     ...
     &VcmxTKattge(tair10, tleafd10) * Vcmax
        ...
        !********************************************************************************************************************************************************************** 
        !Calculate the reference nitrogen use effieciency dependence on CO2 at 25oC - this is a duplicate of below but with hardwired temp and ci:ca ratio
        ...
        Fc  = VcmxTKattge(tgrow, tleaf) * Fc25
        Fj  = JmxTKattge(tgrow, tleaf)  * Fj25
        k_c = Kc25 * exp((79430.0_r8 / (rgas*1.e-3_r8 * 
     &(25.0_r8 + tfrz))) * (1.0_r8 - (tfrz + 25.0_r8) / (tfrz + tleaf)))
        k_o = Ko25 * exp((36380.0_r8 / (rgas*1.e-3_r8 *
     &(25.0_r8 + tfrz))) * (1.0_r8 - (tfrz + 25.0_r8) / (tfrz + tleaf)))
        c_p = Cp25 * exp((37830.0_r8 / (rgas*1.e-3_r8 * 
     &(25.0_r8 + tfrz))) * (1.0_r8 - (tfrz + 25.0_r8) / (tfrz + tleaf)))
        ...
        !tau = exp(-3.949d0 + 28.99d0/(0.00831d0*(tleafd10+273.15)))
        !need to calculate c_p from tau
