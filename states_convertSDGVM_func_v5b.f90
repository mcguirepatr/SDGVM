...
!       gfortran states_convertSDGVM_func_v5b.f90 `nf-config --fflags --flibs`
      ...
      REAL*8, DIMENSION(FYR-SYR+1,NE,DXY,DXY) ::  data_out_SDGVM
      ...
      INTEGER, PARAMETER :: NT = 1172, NV = 14, NV2 = 6
      REAL*8, PARAMETER    :: misval = 1e19
      INTEGER, PARAMETER :: NE2 = 15 
      ...
      DO i=1,NE 
        ...
        CLOSE(15)
        esaarray(i,:,:) = REAL(esaarray_global(i,X0:(X0+DXY-1),Y0:(Y0+DXY-1)),8)
      END DO 
      ...
      PRINT *, 'ESA num_land=',num_land,'num_tot=',DXY*DXY 
      PRINT *, wdh//fname 
      ...
      ! Read the data.
        DO v=1,NV
          CALL CHECK( NF90_GET_VAR(ncid, varid(v), data_in(v,:,:), start=[X0,Y0,t], count=[DXY,DXY,1]) )
      !  print *,'Finished reading data'
          IF( t == SINDEX .AND. v == 1) THEN
            ...
            PRINT *, 'num_land=',num_land,'num_tot=',DXY*DXY 
          END IF
        ...
        IF(compute_next_year) THEN
          ...
          ! end if
           ...
           DO v=1,NVT
            CALL CHECK( NF90_GET_VAR(ncid_t, varid_t(v), data_in_t(v,:,:), start=[X0,Y0,t], count=[DXY,DXY,1]) )
        ...
        IF( t == SINDEX ) THEN
          ...
          PRINT *, 'SDGVM num_land=',num_land,'num_tot=',DXY*DXY
...
!                WRITE(*,FMT='(F9.4)', ADVANCE='no') data_out_SDGVM(year_index,v,1,1)/100.0 
    ...
    FUNCTION JOIN_HYDE(v) RESULT( result_join_hyde ) 
      ...
      IMPLICIT NONE
      REAL*8, INTENT(IN) :: v(15)
      REAL*8, DIMENSION(10) :: result_join_hyde
      REAL*8 :: ov(10), nofcov, nogcov, for_esa, diff, esahyde_cov, hyde_cov, past_esa
