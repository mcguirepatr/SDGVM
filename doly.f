...
*----------------------------------------------------------------------*
      ...
      REAL*8 ftToptV,ftHaV,ftHdV,ftToptJ,ftHaJ,ftHdJ
...
*----------------------------------------------------------------------*
      ...
      if(s070607.eq.1) then
        ...
        IF (sla.GT.0.0d0) THEN
          leafmol = 1.0d0/(sla/0.480d0*12.0d0)
        ELSE
          leafmol =0.0d0
        ENDIF
      endif
...
c Better: should be removed from nppcalc
      amx  = -1000.0d0
      amax = -1000.0d0
...
*      npp_eff = 1.0d0
      ...
      IF (veg) THEN
        CALL NPPCALC(npp_eff,c3,soilc,soiln,minn,soil2g,wtwp,wtfc,
...
*     &12.0*canres*3600.0d0*(24.0d0-hrs)/1000000.0d0
      ...
      !GPP & leaf maintentance respiration 
      ! - can2a is assimilation net of daytime leaf dark respiration
      daygpp= can2a*3600.0d0*hrs/1000000.0d0
      
      !daytime canopy respiration
      ! - canrd is daytime dark respiration and is a function of temperature scaled Vcmax
      ! - dayra is calculated in mol m-2 s-1
      dayra = 0.d0
      !dayra = canrd*3600.0d0*hrs
      
      !To calculate gpp as gross assimilation, i.e. including daytime dark respiration, uncomment the above line
      ! - the original SDGVM calculates GPP net of leaf dark respiration  
      daygpp = daygpp + dayra
      
      !total leaf maintentance respiration is dark respiration plus nighttime canopy respiration
      ! - canres is nighttime respiration and is a function of leaf N
      dayra = dayra + canres*3600.0d0*(24.0d0-hrs)/1000000.0d0

      !GPP net of leaf maintenance respiration - i.e. daily canopy C export
      !daynpp = can2a*3600.0d0*hrs/1000000.0d0 -
      !&canres*3600.0d0*(24.0d0-hrs)/1000000.0d0
      daynpp = daygpp - dayra
   
      !convert daygpp to gC m-2
      ! - why is dayra & daynpp  maintained in mols while daygpp in g?
      !daygpp = daygpp*12.0d0

      !s_gpp & s_lr do nothing so commenting out
...
*----------------------------------------------------------------------*
     ...
     &lchm,ca,site,year,yr0,yrf,speedc,soilc,soiln,mnth,w_scalar,
     &t_scalar,fl,cal)
*----------------------------------------------------------------------*
      IMPLICIT NONE
c     arguments
      REAL*8 ts,tc,avmnpet,avmnppt,avmnt,h2o,flow1,flow2
      REAL*8 slnr,rlnr,scl,rcl,ca,w_scalar,t_scalar
...
*----------------------------------------------------------------------*
     ...
     &rlnr,ar,fms,fmr,scl,rcl,tm,ft,cal,cap,cas,csp,csa,kr,w_scalar,
     &t_scalar)
