...
#  }
  ...
  if(mc){
    ...
    if(daily)   mclapply(dfiles,make_netcdf_TRENDY,mc.cores=procs,daily=daily,fref=fref,...)    
  } else {
...
make_netcdf_TRENDY <- function(varo,annual=F,monthly=F,daily=F,fref='',
  ...
  # annmonth is for variables that are annula outpyut from SDGVM but are required in monthly format for TRENDY
  ...
  ncatt_put(newnc,0,attname='institution',attval='Oak Ridge National Laboratory')
  ncatt_put(newnc,0,attname='history',attval=paste('created:',as.character(as.POSIXlt(Sys.time())),', by: Anthony Walker (walkerap@ornl.gov)'))
  if(!is.null(var$notes)) ncatt_put(newnc,0,attname='notes',attval=var$addnotes)
  if(!is.null(pft))       ncatt_put(newnc,0,attname='PFTs',attval=pft)
  ...
  if(is.null(pft) {
    fname <- paste(fname,'dat',sep='.')  
    newnc <- readSDGVM_writeNCDF(fname,var,newnc,cs,NULL,
                                 lat,lon,nyears,nsites,sa) 
  } else { 
    for( p in pft ) {
      fname <- paste(fname,'_',p,'.dat',sep='')
      newnc <- readSDGVM_writeNCDF(fname,var,newnc,cs,which(pft==p), 
                                   lat,lon,nyears,nsites,sa) 
    }
  }
...
readSDGVM_writeNCDF <- function(fname,var,newnc,cs,pftid=NULL,
                                lat,lon,nyears,nsites,sa) {
  ...
  # scale variable to correct output units
  if(var$namei%in%c('tas'))  df[,cs:length(df)] <- df[,cs:length(df)] + var$scale
  ...
  # put the data into the ncfile
  if(!is.null(pftid)) ncvar_put(newnc,var,da,start=(1,1,pftid,1)
  else                ncvar_put(newnc,var,da)
  #print(levelplot(da[,,101],main=var))
  rm(da)
