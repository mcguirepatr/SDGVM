...
wd <- '/gws/nopw/j04/nexcs/pmcguire/TRENDYv13/db/CO2field/'
setwd(wd)
...
# TRENDY 2023 (2022 dataset)
ifile  <- 'global_co2_ann_1700_2022_TRENDY.txt'
mydata <- read.table(ifile,header=F)
head(mydata)
mydata$Pa <- conv_ppm_to_Pa(mydata[[2]])
names(mydata)
mydata1 <- mydata
mydata1[,3] <- mydata1[1,3] 
write.table(mydata[,c(1,3)],'global_co2_ann_1700_2022_TRENDY.dat',quote=F,row.names=F,col.names=F)
write.table(mydata1[,c(1,3)],'global_co2_ann_1700_2022_TRENDY_1700.dat',quote=F,row.names=F,col.names=F)

# TRENDY 2024 (2023 dataset)
ifile  <- 'global_co2_ann_1700_2023.txt'
mydata <- read.table(ifile,header=F)
head(mydata)
mydata$Pa <- conv_ppm_to_Pa(mydata[[2]])
names(mydata)
mydata1 <- mydata
mydata1[,3] <- mydata1[1,3] 
write.table(mydata[,c(1,3)],'global_co2_ann_1700_2023_TRENDY.dat',quote=F,row.names=F,col.names=F)
write.table(mydata1[,c(1,3)],'global_co2_ann_1700_2023_TRENDY_1700.dat',quote=F,row.names=F,col.names=F)
